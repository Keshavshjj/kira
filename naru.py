...
TOKEN = '7216182705:AAHERDrlYJgr89sh9PjPl8-bYAae4bLvcC4'
MONGO_URI = 'mongodb+srv://yogbe33:yog1122@yogbe33.yz93v.mongodb.net'
FORWARD_CHANNEL_ID = -1002186564814
CHANNEL_ID = -1002186564814
error_channel_id = -1002186564814
...
async def run_attack_command_async(target_ip, target_port, duration, chat_id):
    ...
    # Notify the user when the attack is finished
    bot.send_message(
        chat_id,
        f"*🚨𝑨𝒕𝒕𝒂𝒄𝒌 𝑬𝒏𝒅𝒆𝒅👀\n\n🌐IP: {target_ip}\n📍Port: {target_port}*",
        parse_mode='Markdown'
...
def approve_or_disapprove_user(message):
    ...
    if not is_admin:
        bot.send_message(chat_id, "*𝗢𝗡𝗟𝗬 𝗔𝗗𝗠𝗜𝗡 𝗖𝗔𝗡 𝗨𝗦𝗘 𝗧𝗛𝗜𝗦 𝗖𝗢𝗠𝗠𝗔𝗡𝗗😡*", parse_mode='Markdown')
        return
...
def attack_command(message):
    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "𝘾𝙤𝙣𝙩𝙖𝙘𝙩 𝘼𝙙𝙢𝙞𝙣 𝙁𝙤𝙧 𝘼𝙥𝙥𝙧𝙤𝙫𝙖𝙡👻.")
            return
...
def attack_command(message):
    ...
    try:
        user_data = users_collection.find_one({"user_id": user_id})
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*𝘾𝙤𝙣𝙩𝙖𝙘𝙩 𝘼𝙙𝙢𝙞𝙣 𝙁𝙤𝙧 𝘼𝙥𝙥𝙧𝙤𝙫𝙖𝙡👻.*", parse_mode='Markdown')
            return
...
def send_welcome(message):
    ...
    bot.send_message(message.chat.id, "*👋𝙒𝙚𝙡𝙘𝙤𝙢𝙚 𝙏𝙤 𝘿𝙚𝙢𝙤𝙣 ᴠɪᴘ 𝘿𝙙𝙤𝙨 𝘽𝙤𝙩🗿:*", reply_markup=markup, parse_mode='Markdown')
...
def handle_message(message):
    if message.text == "Instant Plan 🧡":
        ...
        bot.send_message(message.chat.id, "*Please use the following link for Canary Download: https://t.me/+zELM6EtYxDg2YWQ1*", parse_mode='Markdown')
